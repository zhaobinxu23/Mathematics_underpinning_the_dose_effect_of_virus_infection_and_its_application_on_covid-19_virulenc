...
for kkk = 1: 2
    kkk
    for iii = 1:100
       s(1,iii)  = struct('replication_cycle',(9.9 + 0.1*kkk),'waiting_time',0,'time',0);
    end
...
for i = 1:100  %% virus generations
    ...
    d = rand; %% 
    A = s(i,j);
    if d <= (0.60-(0.0012)*i) %% this number = d;
         
        if time_phage+s(i,j).waiting_time >= (9.9 + 0.1*kkk) 
         

       
     
         s(i+1,number+1).replication_cycle = 9.9 + 0.1*kkk;

     
         s(i+1,number+1).waiting_time = time_phage + s(i,j).waiting_time - (9.9 + 0.1*kkk); 

         s(i+1,number+1).time = (10*(i+1));
         
        

         s(i+1,number+2).replication_cycle = 9.9 + 0.1*kkk;


         s(i+1,number+2).waiting_time = time_phage + s(i,j).waiting_time - (9.9 + 0.1*kkk);
